...
bool ColorShaderClass::Init(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitShader(device, hwnd, L"../ConsoleApplication1/ColorVS.hlsl", L"../ConsoleApplication1/ColorPS.hlsl");
	if (!result)
	{
		return false;
	}
...
{
	//Shutdown vertex and pixel shaders as well as the related objects
	ShutDownShader();
	return;
}
...
bool ColorShaderClass::InitShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	if (FAILED(result))
	{
		...
		//if there was nothing in the error message then simply could not find the shader itself
		else
		{
	...
	if (FAILED(result))
	{
		...
		{
			MessageBox(hwnd, psFilename, L"Missing Pixel File", MB_OK);
		}
		return false;
	}
...
void ColorShaderClass::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount, int instanceCount)
{
