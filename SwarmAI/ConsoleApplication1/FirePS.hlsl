...
float4 FirePixelShader(PixelInputType input) : SV_TARGET
{
	...
	//Sample the same noise texture using the three different exture coordinates to get three different noise scales
	...
	//Disstort the three noise x and y coordinates by the hree different istortion x and y values
	...
	//The perturbaion gets stronger as you move up the texture which creaes the flame flickering at the top
	perturb = ((1.0f - input.tex.y) * distortionScale) + distortionBias;

	//Now create the perturbed and distorted texture sampling coordinates that will be used to sample the fire texure
	...
	//Sample the alpha vlue from the alpha texure using the perturbed and distorted texture sampling coordinates
	//This will be used for transparency of the fire
	//Use the clamping sample state instead of he wrap sample state to prevent flames wrapping around
